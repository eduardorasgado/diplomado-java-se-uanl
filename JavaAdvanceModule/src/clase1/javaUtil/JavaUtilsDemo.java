...
/**
 * Implementación de una agenda de 365 dias que guarda una lista de determinadas
 * actividades, por fecha, por 365 dias del year
 * 
...
public class JavaUtilsDemo {
	...
	private static List<String> activities = Arrays
			.asList(new String[] { "Estudiar", "Programar", "Entrenar", "Socializar", "Nadar", "Viajar", "Trabajar",
					"Leer", "Hacer las compras", "Aprender a cocinar", "Dibujar" });
	private static Random generator;

	/**
	 * Tarea 1: Crear la clase demoUtil.java Investigar alguna clase del package
	 * java.util Implementar alguna clase y alguno de sus métodos
	 * 
	 * @param args
	 */
	public static void main(String[] args) {
		...
		for (int i = 0; i < daysInAYear; i++) {
			...
			agenda.put(date.getTime(), new ArrayList<>());

			System.out.println("Date: [" + date.getTime() + "] Added Automatically");
		}

		System.out.println("----------------------------------------------------------------------------\n");

		agenda.keySet().forEach(agendaDate -> {
			agenda.get(agendaDate).addAll(getActivityList(agendaDate));
		});
		...
		agenda.keySet().forEach(agendaDate -> {
			...
			System.out.println("\n[Activity for " + calendarInstance.get(Calendar.DAY_OF_MONTH) + " of "
					+ calendarInstance.getDisplayName(Calendar.MONTH, Calendar.LONG, Locale.ENGLISH) + " of "
					+ calendarInstance.get(Calendar.YEAR));

			agenda.get(agendaDate).forEach(activity -> System.out.println("	[*] " + activity));
		});
